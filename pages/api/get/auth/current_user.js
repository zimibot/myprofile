...
export default authenticated(async function getCurrentUser(req, res) {
    try {
        ...
        const currentSettings = await UsersSettings.findOne({
            where: {
                nim: data.nim
            },
        });
        ...
        const users = await Users.findAll({
            ...
            attributes: ["email", "alamat", "foto", "fullname", "nim", "nohp", "updatedAt", "createdAt", "status", "username", "twit", "fb", "instagram", "job"],
            include: [{
                model: UsersSettings,
